...
int main(int argc, char *argv[]) {
  ...
  if (argc < 4) {
    fprintf(stderr, "Usage: %s gfxname fontfile size [first] [last]\n", argv[0]);
    return 1;
  }
...
/*
  ptr = strrchr(argv[2], '/'); // Find last slash in filename
  if (ptr)
    ptr++; // First character of filename (path stripped)
  else
    ptr = argv[2]; // No path; font in local dir.*/
...
*/
  gfxFontName = malloc(20);
  strcpy(gfxFontName, argv[1]);
  ...
  if ((err = FT_New_Face(library, argv[2], 0, &face))) {
